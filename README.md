...
Khuyến nghị: chuyển credential thật sang **`include/secrets.h`** (file local, không commit). `app_config.h` sẽ tự include file `secrets.h` nếu tồn tại trong include path của project.
